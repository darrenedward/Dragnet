...
**Note:** Specs live in `.agent-os/specs/` (with the dot prefix — this is the project convention per `CLAUDE.md`). If the directory doesn't exist, create it when saving the spec folder. The path reference in this command's earlier examples (`agent-os/specs/`) is the upstream agent-os default; this project overrides it to `.agent-os/specs/`.
...
- **tasks.md** — Phase-grouped checkbox list (run `/agent-os:write-tasks {folder-name}` after this task completes to generate it)
- **visuals/** — Any mockups or screenshots provided (optional)
...
├── tasks.md          # Phase-grouped checkboxes (generated by /agent-os:write-tasks)
└── visuals/          # Mockups, screenshots (if any)
```

After shaping completes, the typical workflow is:

1. `/agent-os:shape-spec` — produces plan/shape/standards/references (this command).
2. `/agent-os:write-tasks {folder}` — generates tasks.md from plan.md.
3. `/agent-os:next-task {folder}` — implements the next undone task, flips the checkbox, commits. Re-invoke to loop.
4. `/agent-os:verify-spec {folder}` — runs lint/test/build, reports per-phase status.
