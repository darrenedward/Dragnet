...
## Queue + merge contract (read first)

### Vocabulary

| Phrase | Means | Does **not** mean |
|---|---|---|
| **Scan finished** | `ReviewRun.status === "completed"` (or PR badge `Completed`) | Merge-ready / ship it |
| **Merge ready** | Shared `isMergeReady` gate returned `mergeReady: true` | Rating alone, or scan finished |
| **Blocked** | Prelude/gate refused work (`Blocked at {gate}` / gate code) | Soft “not ready” after a finished scan |
| **Not ready** | Scan finished (or absent) but `mergeReady: false` with a reason | Pipeline still running |

`Completed` ≠ merge-ready. Null rating, skipped outcome, rating &lt; 8, incomplete reliability, refused, or stale → **not merge-ready**.

### Explicit admit vs AFK auto-rescan

| Path | What admits a scan job |
|---|---|
| **Explicit** — UI Run / Force, `/dragnet` `prcheck` / fix recheck, prepush | **Always admits** (or joins an in-flight job for the same PR). Auto-rescan disabled does **not** block these. |
| **AFK** — webhook, poller, hosted automatic | Gated by **auto-rescan policy only**. When auto-rescan is off, background paths must not enqueue. |

When `prcheck` returns `Accepted` with `jobId` / `queuePosition`, the review is **queued/running** — not finished. Poll `prcheckstatus` until terminal. Do not treat HTTP 202 / Accepted as a successful review.

### Merge readiness (shared rule)

`/dragnet merge`, prepush, findings API, and PR header share one helper (`isMergeReady` in `src/lib/mergeReady.ts`):
...
- `mergeReady` (boolean)
- `mergeBlockReason` (stable code)
- `mergeReadyMessage` (human text)

Markdown in `message` also includes `**Merge ready:** yes|no (…)`. Prefer the JSON fields.
...
| `/dragnet merge [n,n,...]` | Candidates must pass shared **`isMergeReady`** (not rating folklore). Stack topology via `gh`, strategy via AskUserQuestion, merge on approval. Protocol: [references/merge.md](./references/merge.md). **Mutating.** |
...
10. **Large PR advisory — warn, don't split.** After `prcheck` completes and before rendering findings, check the PR's size against engine Review Limits (Settings → Review Limits / `.dragnet/review-limits.json`). **Shipped defaults** (live installs may override): normal &lt; **800** diff lines / **40** code files; approaching oversized **800–3000** / **40–100**; oversized &gt; **3000** / **100**. These are **diff lines + code-file counts**, not authoring LOC (no 500 LOC rule). UI size badges (`prSizeProfile`) are a separate glance residual.

    | Tier | Code lines (default) | Code files (default) | Action |
    |---|---|---|---|
...
```json
{
  ...
  "stability": {
    ...
    "readyToMerge": false,    // STREAK HINT ONLY — not the merge gate; use mergeReady
...
**Merge-ready rendering (required):** always surface the shared gate next to the score:

```
Scan finished · Merge ready: yes
```

or

```
Scan finished · Not ready (rating_below_threshold) — Rating 7/10 is below the merge threshold (8+).
```

or when blocked before a finished run:

```
Blocked / not finished — mergeReady: false (no_run | not_finished)
```

- Prefer `mergeReady`, `mergeBlockReason`, `mergeReadyMessage` from the JSON body.
- `productionScore` is display-only; never equate it with merge permission.
- `stability.readyToMerge` / `weightedReadyToMerge` are **streak hints only** — they are not the merge gate.

**Stability rendering:** when `stability` is present, render under the rating trend:

```
✓ Stable streak — 3 consecutive clean rounds (hint only; still require mergeReady)
```
...
- If `ratingTrend.length < 2`, render `○ Gathering data — only 1 scan completed`
