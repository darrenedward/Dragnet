...
Stack-aware merge orchestrator. Candidates must pass Dragnet's **shared `isMergeReady` gate** (same rule as prepush, findings API, and the PR header) — not rating-only folklore. Then walk stack topology, surface reality/problems, present strategy options via AskUserQuestion, verify live `gh` state, and execute merges on approval.

**Vocabulary:** *Scan finished* (`status: completed`) ≠ *Merge ready*. *Blocked* means a prelude/gate refused work. *Not ready* means a finished (or missing) run failed `isMergeReady`.

**Shared merge rule** (`src/lib/mergeReady.ts` — do not reimplement):
...
- `/dragnet merge` — list PRs with `mergeReady: true`, analyze stacks, present strategy options
...
**Source A (Dragnet merge gate):** For each PR of interest, POST `prcheckstatus <id>` (or use findings). Require **`mergeReady === true`**. Do **not** filter on `rating >= 8` alone, `productionScore`, or `stability.readyToMerge` / `weightedReadyToMerge` — those are hints. If `mergeReady` is false, record `mergeBlockReason` / `mergeReadyMessage` in the Reality/Problem table and exclude from default merge strategies (unless `--force` soft-override is in play for *other* soft gates — `mergeReady: false` for null rating / skipped / incomplete reliability still surfaces as not merge-ready).

**Source B (gh authoritative stack + CI):**
```bash
gh pr list --state open --json number,title,baseRefName,headRefName,mergeable,isDraft,statusCheckRollup,reviewDecision
```

Cross-reference by `headRefName == branch` (Dragnet) ↔ `headRefName` (gh). Build the **live** stack graph from gh's `baseRefName`/`headRefName` edges — this is authoritative for stack order. Dragnet's `stackDepth` is a sanity check; if the two disagree, gh wins.

If args present (`1` or `1,7,8` or `1-4`), filter the candidates to those ordinals (translated via `prlist` ordering). Else take all with `mergeReady: true`.
...
- `mergeReady: false` with a reason that is not a hard pipeline block (still **show** the reason; `--force` only proceeds after AskUserQuestion warns that Dragnet's shared gate failed)
...
Without `--force`, surface these as "deferred: soft gate." With `--force`, override with explicit warning that the user accepts the risk. **Never** invent a pass from `rating >= 8` when `mergeReady` is false.
...
- **No PRs match the filter:** render "No PRs are merge-ready (`isMergeReady`) — nothing to merge" and exit. Don't prompt with empty strategies. If some PRs only have high ratings but `mergeReady: false`, list them under "Scan finished but not merge-ready" with reasons.
