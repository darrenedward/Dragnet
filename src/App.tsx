...
export default function App() {
  ...
  return (
    ...
    <div className="flex flex-col h-screen w-full bg-[#0B0E14] text-slate-300 font-sans select-none overflow-hidden relative">
      ...
      {/* API key creation banner — shown once after a new project is registered */}
      {d.createdApiKey && (
        <div className="fixed bottom-6 right-6 z-50 bg-[#0F1219] border border-amber-500/30 rounded-xl p-4 shadow-2xl max-w-md">
          <div className="flex items-center gap-2 mb-2">
            ...
            <button
              onClick={() => d.setCreatedApiKey(null)}
              className="ml-auto p-1 hover:bg-white/10 rounded-lg text-slate-400 hover:text-white transition-colors cursor-pointer"
            >
              <X size={14} />
            </button>
          </div>
          ...
          <p className="text-[9px] text-slate-600 font-mono mt-1.5">Prefix: {d.createdApiKey.prefix} — Won't be shown again.</p>
        </div>
      )}
