...
export default function PrHeader({
  ...
  const cloneFailed = isCloneFailedForActions(resolvedSeam);
  const runDisabled = cloneFailed || scanning || !repoIndexedAt;

  const chips = buildLayoutCChips({
    ...resolvedSeam,
    prStatus: activePR.status,
    sizeTier: activePR.sizeProfile?.tier ?? null,
    mergeReady: mergeReady ?? null,
    mergeMessage: mergeReadyMessage ?? null,
