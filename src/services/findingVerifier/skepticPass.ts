...
export async function runSkepticPass(
  ...
  const batch = candidates.slice(0, SKEPTIC_BATCH_CAP);
  const truncatedCount = candidates.length - batch.length;
  if (truncatedCount > 0) {
    console.warn(
      `[skeptic] truncating batch to ${SKEPTIC_BATCH_CAP} of ${candidates.length} findings (cap)`,
    );
  }
